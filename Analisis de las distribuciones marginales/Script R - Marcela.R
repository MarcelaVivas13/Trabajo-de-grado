...
###### graficos enfermos no enfermos monocitos

x11()
par(mfrow=c(2,2), oma=c(2,1,2,1))
hist(Datos$nmono,freq=F,ylim = c(0,1.4),breaks=10, 
     main ="Histograma y densidades teoricas",xlab = "data",col = "white")
curve(dlnorm3(x, gamma = -0.01347245,meanlog = -0.40325803,sdlog =0.53709532), 
      col = "red", lwd = 1, add = TRUE)
...
# Grafica el Q-Q plot
plot(cuantiles_nig, cuantiles_datos, main = "QQ-plot", xlab = "Theoretical quantiles",
     ylab = "Empirical quantiles", col = "red")
abline(0, 1, col = "black") # Agrega una línea de referencia para una distribución perfecta
...
mtext("Monocitos individuos enfermos", side=3, line=-1, at=0.5, cex=1.3, outer=TRUE)
mtext("Monocitos individuos no enfermos", side=1, line=-18, at=0.5, cex=1.3, outer=TRUE)
